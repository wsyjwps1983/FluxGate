...
class SiliconFlowLLM(BaseLLM):
    """LLM for SiliconFlow. Requires a SiliconFlow API key from https://siliconflow.cn."""
    ...
    def __init__(
        ...
        :param name: The name of the SiliconFlow model to use.
        :type name: Optional[str]
        :param siliconflow_api_key: The SiliconFlow API key.
        :type siliconflow_api_key: Optional[str]
        :param siliconflow_base_url: The base URL for SiliconFlow API.
        :type siliconflow_base_url: Optional[str]
        ...
        """
        if name is None:
            name = "deepseek-ai/deepseek-r1"  # 默认使用DeepSeek-R1模型
        ...
        if api_key is None:
            raise ValueError("SiliconFlow API key cannot be 'None'.")
        try:
    ...
    def __call__(
        ...
        """Call the SiliconFlowLLM.

        :param messages: The messages to pass to the SiliconFlowLLM.
        :type messages: List[Message]
        :param function_schemas: The function schemas to pass to the SiliconFlowLLM.
        :type function_schemas: Optional[List[Dict[str, Any]]]
        :return: The response from the SiliconFlowLLM.
        :rtype: str
        """
        ...
        try:
            ...
            if function_schemas:
                ...
                output = str(
                    self._extract_tool_calls_info(tool_calls)  # type: ignore
    ...
    async def acall(
        ...
        """Call the SiliconFlowLLM asynchronously.
        ...
        if self._async_client is None:
            raise ValueError("SiliconFlow async_client is not initialized.")
        try:
            tools: Union[List[Dict[str, Any]], NotGiven] = (
            ...
            if function_schemas:
                ...
                # Collecting multiple tool calls information
                output = str(
                    await self.async_extract_tool_calls_info(tool_calls)  # type: ignore
                )  # str in keeping with base type.
            else:
                content = completion.choices[0].message.content
                if content is None:
...
def get_schemas_siliconflow(items: List[Callable]) -> List[Dict[str, Any]]:
    """Get function schemas for the SiliconFlow LLM from a list of functions.

    :param items: The functions to get function schemas for.
    :type items: List[Callable]
    :return: The schemas for the SiliconFlow LLM.
    :rtype: List[Dict[str, Any]]
    """
    schemas = []
    for item in items:
