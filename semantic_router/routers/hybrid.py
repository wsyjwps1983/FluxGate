...
class HybridRouter(BaseRouter):
    ...
    def _set_score_threshold(self):
        ...
        Additionally, this method ensures that each route has its own score threshold
        ...
        # Ensure each route has a score threshold
    ...
    def add(self, routes: List[Route] | Route):
        ...
        # Set threshold for each new route if not already set
        for route in routes:
            if route.score_threshold is None:
                route.score_threshold = self.score_threshold
